...
/***/ function(module, exports, __webpack_require__) {
	...
	var _webtendo = __webpack_require__(1);
	...
	var createLetterDiv = function createLetterDiv(letter, index) {
	  var id = "letter_" + index;
	  var $letter = (0, _jquery2.default)("<div />", {
	...
	var getLetterFromDiv = function getLetterFromDiv($div) {
	  var params = $div.attr('data-buttonvalue').split(',');
	  return _underscore2.default.filter(params, function (p) {
	    return p.split('=')[0] == 'letter';
	  })[0].split('=')[1];
	};

	var selectLetter = function selectLetter(id) {
	  _underscore2.default.each((0, _jquery2.default)("#hand").children(), function (letterDiv) {
	    if ((0, _jquery2.default)(letterDiv).attr('id') == id) {
	      (0, _jquery2.default)(letterDiv).addClass('selected');
	    } else {
	      (0, _jquery2.default)(letterDiv).removeClass('selected');
	    }
	  });
	};

	var playLetter = function playLetter() {
	  var $selected = (0, _jquery2.default)("#hand .selected");
	...
	var finishTurn = function finishTurn() {
	  dataChannels[clientId].send(JSON.stringify({
	    finish_turn: true
	  }));
	};
	...
	var createScrabbleBoard = function createScrabbleBoard() {
	  var rows = 15;
	  var columns = 15;
	  ...
	  for (var i = 0; i < columns; i++) {
	    var $square = (0, _jquery2.default)("<div />", {
	      class: 'square col_' + i
	    });
	    $row.append($square);
	  }
	  for (var i = 0; i < rows; i++) {
	    ...
	    (0, _jquery2.default)("#main").append($currRow);
	  }
	};

	function updateScrabbleBoard() {
	  _underscore2.default.each(board, function (r, x) {
	    _underscore2.default.each(r, function (c, y) {
	      if (!_underscore2.default.isUndefined(c) && !_underscore2.default.isNull(c)) {
	        var $tile = (0, _jquery2.default)('.row_' + y + ' > .col_' + x);
	        $tile.text(c.letter);
	...
	var addPlayer = function addPlayer(id) {
	  if (id in players) {
	    return;
	  }
	  players[id] = { color: playerColors[playerList.length] };
	  playerList.push(id);
	  (0, _jquery2.default)(".player_list").empty();
	  _underscore2.default.each(playerList, function (player, index) {
	    (0, _jquery2.default)(".player_list").append("<li style='color: " + players[player].color + "'>" + player + "</li>");
	  });
	  if (playerList.length >= 2) {
	    (0, _jquery2.default)(".start_button").show();
	  }
	};
	...
	function selectTile(x, y) {
	  ...
	  (0, _jquery2.default)(".row_" + y + " > .col_" + x).addClass('selected');
	}

	function nextPlayer() {
	  currentPlayer = (currentPlayer + 1) % playerList.length;
	  (0, _jquery2.default)(".curr_player").text(playerList[currentPlayer]);
	  resetPosition();
	  currWordPositions = [];
	}
	...
	var playLetter = function playLetter(player, letter, index) {
	  // check that the spot isn't already taken
	  // TODO: Make it so the joystick never ends in an unavailable position.
	  if (!_underscore2.default.isUndefined(board[position.x][position.y])) {
	...
	var handleJoystickMoved = function handleJoystickMoved(x, y) {
	  if (x == 0 && y == 0) {
	...
	var hasGameStarted = function hasGameStarted() {
	  return gameStarted;
	};
	...
	(0, _jquery2.default)(".start_button").click(function () {
	  start();
	});
...
/***/ function(module, exports, __webpack_require__) {
	...
	(function () {
	  // Utils.
	  var logging = __webpack_require__(59).log;
	  var browserDetails = __webpack_require__(59).browserDetails;
	  // Export to the adapter global object visible in the browser.
	  module.exports.browserDetails = browserDetails;
	  module.exports.extractVersion = __webpack_require__(59).extractVersion;
	  module.exports.disableLog = __webpack_require__(59).disableLog;
	  ...
	  var firefoxShim = __webpack_require__(65) || null;
	  var safariShim = __webpack_require__(67) || null;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var SDPUtils = __webpack_require__(63);
	var browserDetails = __webpack_require__(59).browserDetails;
