...
let storage = IS_LOCAL ? window.sessionStorage : window.localStorage
...
try {
  ...
  setTimeout(() => {
    try {
      ...
      window.alert('This browser is not supported. Please use Android Chrome or iOS native app.')
    }
  }, 500)
}
...
function attachListeners (socket) {
  ...
  socket.on('full', function (room) {
    ...
    window.alert(`Room ${room} looks full`)
  })
  ...
  socket.on('nohost', room => {
    ...
    window.alert(`No host for room ${room}. Please open a host on your laptop.`)
  })
...
function onDataChannelCreated (channel) {
  ...
  channel.onmessage = (event) => {
    ...
    if (x.action === 'echo') {
      ...
      var str = 'round trip latency ' + (window.performance.now() - x.time).toFixed(2) + ' ms'
      // maybeLog()(str);
